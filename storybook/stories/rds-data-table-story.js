...
import RdsDataTableNotesMD from '../../core/components/rds-data-table/rds-data-table-notes.md';
import {
  RdsDataTable,
  RdsDataTableAction,
  RdsDataTableRow,
...
const storiesDefault = storiesOf('Components/RdsDataTable', module);
// const storiesExperimental = storiesOf('Experimental/RdsDataTable', module);
...
let preKnobs = {
  ...
  actions: {
    ...
    slot: 'actions',
    value: `
<rds-data-table-action @click="action1">
...
</rds-data-table-action>
<rds-data-table-action @click="action2">
...
<rds-data-table-action @click="action3">
...
<rds-button small @click="actionNew">Add new</rds-button>
      `,
  },
  batchActions: {
    group: 'slots',
    slot: 'batch-actions',
    value: `
<rds-button @click="onBatchAction1">
  Delete
  <TrashCan16 class="bx--btn__icon"/>
</rds-button>
<rds-button @click="onBatchAction2">
  Save
  <Save16 class="bx--btn__icon"/>
</rds-button>
<rds-button @click="onBatchAction3">
  Download
...
`,
  ...
  slottedHeadings: {
    ...
    slot: 'headings',
    value:
      '\n    <rds-data-table-heading heading="Name" sortable />' +
      '\n    <rds-data-table-heading>Protocol<rds-tooltip direction="bottom" tip="How comms get done" /></rds-data-table-heading>' +
      '\n    <rds-data-table-heading><rds-tooltip direction="bottom" tip="The port number">Port</rds-tooltip></rds-data-table-heading>' +
      '\n    <rds-data-table-heading heading="Rule" />' +
      '\n    <rds-data-table-heading heading="Attatched Groups" />' +
      '\n    <rds-data-table-heading heading="Status" />' +
      '\n',
  },
  slottedData: {
    group: 'slots',
    slot: 'data',
    value:
      '\n    <rds-data-table-row v-for="(row, rowIndex) in internalData" :key="`${rowIndex}`" :value="`${rowIndex}`" :aria-label-for-batch-checkbox="`Custom aria label for row ${rowIndex} batch`">' +
      '\n       <rds-data-table-cell v-for="(cell, cellIndex) in row" :key="`${cellIndex}`" :value="`${cellIndex}`" v-html="cell"></rds-data-table-cell>' +
  ...
  expandingSlottedData: {
    ...
    slot: 'data',
    value:
      '\n    <rds-data-table-row v-for="(row, rowIndex) in internalData" :key="`${rowIndex}`" :value="`${rowIndex}`" :expanded="rowIndex === rowExpanded" aria-label-expand-row="Go large" aria-label-collapse-row="Go small">' +
      '\n       <rds-data-table-cell v-for="(cell, cellIndex) in row" :key="`${cellIndex}`" :value="`${cellIndex}`" v-html="cell"></rds-data-table-cell>' +
  ...
  htmlData: {
    ...
    slot: 'data',
    value:
      '\n    <rds-data-table-row v-for="(row, rowIndex) in [`ibm`, `beta`, `local`, `custom`, `private`]" :key="`${rowIndex}`" :value="`${rowIndex}`">' +
      '\n       <rds-data-table-cell><input type="text" :value="row" style="border: none; background: none; width: 100%;"/></rds-data-table-cell>' +
      '\n       <rds-data-table-cell><input type="number" :value="rowIndex * rowIndex" style="border: none; background: none; width: 100%;" /></rds-data-table-cell>' +
      '\n       <rds-data-table-cell><input type="password" value="ASecret" style="border: none; background: none; width: 100%;" /></rds-data-table-cell>' +
      '\n       <rds-data-table-cell><a href="http://vue.carbondesignsystem.com">Here</a></rds-data-table-cell>' +
      '\n       <rds-data-table-cell><rds-tag kind="red" label="I am a tag" /></rds-data-table-cell>' +
      '\n       <rds-data-table-cell><rds-button type="button" v-html="`Clicky ${row}`" style="width: 100%;"></rds-button></rds-data-table-cell>' +
      '\n       <rds-data-table-cell>' +
      '\n           <rds-overflow-menu flip-menu style="margin: 0 auto;">' +
      '\n               <rds-overflow-menu-item>Edit</rds-overflow-menu-item>' +
      '\n               <rds-overflow-menu-item>Test</rds-overflow-menu-item>' +
      '\n           </rds-overflow-menu>' +
      '\n       </rds-data-table-cell>' +
      '\n    </rds-data-table-row>\n',
  },
...
<rds-data-table${settings.group.attr} ${settings.group.slots.indexOf('slot="data"') < 0 ? ':data="filteredData"' : ''} ${
        settings.group.attr.indexOf(':overflow-menu=') < 0 ? '' : '@overflow-menu-click="onOverflowMenuClick"'
      } ${settings.group.slots.indexOf('rds-overflow-menu') < 0 ? '' : ':overflow-menu="true"'} ref="table">${
  ...
  `;
    ...
    {
      notes: { markdown: RdsDataTableNotesMD },
    }
  );
}
...
for (const story of storySet) {
  storiesDefault.add(
    ...
    () => {
      ...
      const templateString = `<rds-data-table-skeleton${settings.group.attr}></rds-data-table-skeleton>`;

      return {
        components: { SvTemplateView, RdsDataTableSkeleton },
        template: `
        <sv-template-view
          sv-margin
          sv-position="center"
