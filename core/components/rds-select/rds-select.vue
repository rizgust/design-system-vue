<template>
  <rds-wrapper :tag-type="formItem ? 'div' : ''" :class="`rds-select ${carbonPrefix}--form-item`">
    <div
      :data-test="value"
      :class="[
        `${carbonPrefix}--select`,
    ...
    >
      ...
      <rds-wrapper :tag-type="inline ? 'div' : ''" :class="`${carbonPrefix}--select-input--inline__wrapper`">
        ...
        <!-- rds-wrapper div ${carbonPrefix}--select-input--inline__wrapper -->
      </rds-wrapper>
    </div>
    <!-- rds-wrapper div ${carbonPrefix}--form-item -->
  </rds-wrapper>
</template>
...
export default {
  name: 'RdsSelect',
  inheritAttrs: false,
  components: { RdsWrapper, ChevronDownGlyph, WarningFilled16 },
  ...
  props: {
    ...
    multiple: {
      type: String,
      validator: () => {
        console.warn('property multiple not supported in RdsSelect');
        return false;
      },
    },
