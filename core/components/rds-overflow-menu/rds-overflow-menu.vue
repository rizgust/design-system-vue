<template>
  <div data-overflow-menu :class="`rds-overflow-menu ${carbonPrefix}--overflow-menu`" :id="uid">
    <button
      :class="[
    ...
    >
      <div
        class="rds-overflow-menu__before-content"
        ref="beforeContent"
        tabindex="0"
      ...
      <div
        class="rds-overflow-menu__after-content"
        ref="afterContent"
        tabindex="0"
        style="position: absolute; height: 1px; width: 1px; left: -9999px;"
...
export default {
  ...
  created() {
    this.$on('rds:close', this.doClose);
    this.$on('rds:click', this.menuItemclick);
  },
  methods: {
    checkFocusOut(ev) {
      if (this.open) {
      ...
      if (this.open) {
        ...
        for (let tryOn of focusOnList) {
          if (
            ...
            !(
              tryOn.classList.contains('rds-overflow-menu__before-content') ||
              tryOn.classList.contains('rds-overflow-menu__after-content') ||
              tryOn.tabindex < 0
            )
          ) {
            focusOn = tryOn;
