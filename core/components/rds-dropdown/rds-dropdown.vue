<!--
  ...
  Usage:
    <rds-dropdown placeholder="Choose an option">
  ...
  <div :class="{ [`${carbonPrefix}--form-item`]: formItem }" :id="uid">
    ...
    >
      ...
      >
        ...
        >
          <slot>
            <rds-dropdown-item v-for="item in items" v-bind:key="item" :value="item">{{ item }}</rds-dropdown-item>
          </slot>
        </ul>
      </div>
...
import RdsDropdownItem from './rds-dropdown-item';
...
export default {
  ...
  components: { WarningFilled16, ChevronDown16, RdsDropdownItem },
  props: {
    disabled: Boolean,
    ...
    items: {
      ...
      validator(arr) {
        if (!Array.isArray(arr)) {
          console.warn('RdsDropdown - items must be in array');
        }
        return arr;
      },
    },
  ...
  methods: {
    ...
    dropdownItems() {
      return this.$children.filter(item => item.$_RdsDropdownItem);
    },
    doMove(up) {
      // requery could have changed
      let currentFocusEl = this.$el.querySelector('.rds-dropdown-item :focus');
