<template>
  <rds-wrapper :tag-type="formItem ? 'div' : ''" :class="`rds-date-picker ${carbonPrefix}--form-item`" :id="uid">
    <div
...
export default {
  ...
  components: { Calendar16, RdsWrapper },
  props: {
    ...
    invalid: /* deprecate */ {
      ...
      validator(val) {
        if (val !== undefined && process.env.NODE_ENV === 'development') {
          console.warn('RdsDatePicker: invalid prop deprecated in favour of invalidMessage');
        }
        return true;
      },
    },
    invalidDateMessage: /* deprecate */ {
      ...
      validator(val) {
        if (val !== undefined && process.env.NODE_ENV === 'development') {
          console.warn('RdsDatePicker: invalidDateMessage deprecated in favour of invalidMessage');
        }
        return true;
      },
    },
      ...
      if (this.isRange) {
        ...
        if (firstDate !== this.$refs.date.value) {
          ...
          if (this.$listeners['simpleChange'] && process.env.NODE_ENV === 'development') {
            console.warn('RdsDatePicker: simple change event deprecated in favour of change.');
