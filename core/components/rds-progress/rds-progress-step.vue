<template>
  <li
    :class="[
      `rds-progress-step ${carbonPrefix}--progress-step`,
...
export default {
  ...
  mounted() {
    this.$_RdsProgressStep = true; // for use by parent with $children
    this.$parent.$emit('rds:mounted');
  },
  beforeDestroy() {
    this.$parent.$emit('rds:beforeDestroy');
  },
  watch: {
    complete() {
      this.$parent.$emit('rds:completed');
    },
  },
  computed: {
    internalState: {
