<template>
  <div :class="`rds-definition-tooltip ${carbonPrefix}--tooltip--definition ${carbonPrefix}--tooltip--a11y`" :id="uid">
    <button
  ...
  props: {
    ...
    direction: {
      ...
      validator(val) {
        ...
        if (!valid) {
          console.warn(`RDSDefinitionTooltip.direction must be one of the following: ${validValues}`);
        }
        return valid;
      },
    },
