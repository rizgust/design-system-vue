<template>
  <rds-form-item class="rds-file-uploader">
...
import RdsFormItem from '../rds-form/rds-form-item';
...
export default {
  name: 'RdsFileUploader',
  components: { RdsFormItem, CheckmarkFilled16, WarningFilled16, Close16, RdsWrapper },
    ...
    kind: {
      ...
      validator: val => {
        ...
        if (!validValues.includes(val)) {
          console.warn(`RdsFileUploader: valid values for 'kind' are ${validValues}`);
        }
        return true;
      },
    },
    ...
    buttonLabel: {
      ...
      validator: val => {
        if (val !== undefined && process.env.NODE_ENV === 'development') {
          console.warn('RdsFileUploader: button-label prop deprecated in favour of drop-target-label');
        }
        return true;
      },
    },
