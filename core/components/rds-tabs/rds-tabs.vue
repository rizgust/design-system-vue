<template>
  <div class="rds-tabs" ref="tabs" style="width: 100%;">
    <div
      data-tabs
      :class="[`rds-tab ${carbonPrefix}--tabs--scrollable`, { [`${carbonPrefix}--tabs--container`]: container }]"
      role="navigation"
      v-on="$listeners"
      v-bind="$attrs"
    ...
    >
      ...
      <ul :class="`${carbonPrefix}--tabs--scrollable__nav`" role="tablist" ref="tablist">
        <li
          ...
          :key="tab.uid"
          :class="[
            `rds-tabs-button  ${carbonPrefix}--tabs--scrollable__nav-item`,
            {
...
export default {
  ...
  created() {
    // add these on created otherwise rds:mounted is too early.
    ...
    this.$on('rds:selected', srcComponent => this.onRdsSelected(srcComponent));
    this.$on('rds:disabled', srcComponent => this.onRdsDisabled(srcComponent));
    this.$on('rds:enabled', srcComponent => this.onRdsEnabled(srcComponent));
  ...
  methods: {
    ...
    onRdsMount(srcComponent) {
      ...
      if (this.tabs.filter(item => item.uid === srcComponent.uid).length > 1) {
        console.error(`RdsTabs: Duplicate ID specified for RdsTab, this may cause issues. {id: ${srcComponent.id}}}`);
      }
    ...
    onRdsSelected(srcComponent) {
      this.doTabClick(srcComponent.uid);
    },
    onRdsDisabled(srcComponent) {
      this.disabledTabs.push(srcComponent.uid);
    },
    onRdsEnabled(srcComponent) {
    ...
    checkDisabled(srcComponent) {
      if (srcComponent.internalDisabled) {
        this.onRdsDisabled(srcComponent);
      } else {
        this.onRdsEnabled(srcComponent);
      }
    },
    checkSelected() {
      let id;
