<template>
  <div
    :class="[
      `rds-multi-select ${carbonPrefix}--multi-select__wrapper ${carbonPrefix}--list-box__wrapper`,
      {
...
import RdsTag from '../rds-tag/rds-tag';
...
export default {
  ...
  components: { WarningFilled16, ChevronDown16, RdsCheckbox, RdsTag, Close16 },
  props: {
    autoFilter: Boolean,
    ...
    options: {
      ...
      validator(list) {
        ...
        if (!result) {
          console.warn('RdsMultiSelect - all options must have name, label and value');
        }
        return result;
      },
    },
    selectionFeedback: {
      ...
      validator(val) {
        if (!selectionFeedbackOptions.includes(val)) {
          console.warn(`RdsMultiSelect: invalid selectionFeedback "${val}", use one of ${selectionFeedbackOptions}`);
          return false;
        }
        return true;
      },
