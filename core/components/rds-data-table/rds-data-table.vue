<template>
  <div :style="tableStyle" class="rds-data-table">
    <div :class="`${carbonPrefix}--data-table-container`">
      ...
      <section v-if="hasToolbar" :class="`${carbonPrefix}--table-toolbar`">
        ...
        >
          <div :class="`${carbonPrefix}--action-list`">
            ...
            <rds-button :class="`${carbonPrefix}--batch-summary__cancel`" size="small" @click="deselect">{{
              batchCancelLabel
            }}</rds-button>
          </div>
      ...
      >
        ...
        <rds-wrapper :tag-type="hasExpandables ? '' : 'tbody'">
          <slot name="data">
            <rds-data-table-row
            ...
            >
              ...
              >
                <rds-wrapper :tag-type="skeleton ? 'span' : ''">{{ cell }}</rds-wrapper>
              </rds-data-table-cell>
            </rds-data-table-row>
          </slot>
        </rds-wrapper>
...
<script>
import RdsDataTableHeading from './rds-data-table-heading';
import RdsDataTableRow from './rds-data-table-row';
import RdsDataTableCell from './rds-data-table-cell';
import RdsButton from '../rds-button/rds-button';
import RdsCheckbox from '../rds-checkbox/rds-checkbox';
import RdsPagination from '../rds-pagination/rds-pagination';
...
export default {
  ...
  created() {
    ...
    this.$on('rds:sort', (srcComponent, value) => this.onSort(srcComponent, value));
  },
  mounted() {
  ...
  methods: {
    ...
    onRdsMount(thing) {
      if (thing.$_RdsDataTableHeading) {
        this.registeredHeadings = this.$children.filter(item => item.$_RdsDataTableHeading);
        const heading = thing;
        if (this.registeredHeadings.filter(item => item.uid === heading.uid).length > 1) {
          console.error(
            `RdsDataTable: Duplicate ID specified for RdsDataTableHeading, this may cause issues. {id: ${heading.id}}`
          );
        }
      } else {
        const row = thing;
        ...
        if (this.registeredRows.filter(item => item.uid === row.uid).length > 1) {
          console.error(
            `RdsDataTable: Duplicate ID specified for RdsDataTableRow, this may cause issues. {id: ${row.id}, value: ${row.value}}`
          );
        }
        row.$on('rds:expanded-change', this.onRdsExpandedChange);
        this.updateSomeExpandingRows();
      }
    },
    onRdsBeforeDestroy(thing) {
      if (thing.$_RdsDataTableHeading) {
        this.registeredHeadings = this.$children.filter(item => item.$_RdsDataTableHeading);
      } else {
        const row = thing;
    ...
    updateRowsSelected() {
      ...
      this.headingChecked =
        this.dataRowsSelected.length === this.registeredRows.filter(item => item.isRdsDataTableRow).length;
