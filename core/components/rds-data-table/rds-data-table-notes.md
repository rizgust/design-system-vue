...
```html
<rds-data-table :columns="columns" :data="filteredData"></rds-data-table>
```
...
Instead of supply data as two dimensional array the user can supply slotted content using the RdsDataTableRow and RdsDataTableCell components.

The RdsDataTableCell component can also take slotted content which allowing the freedom to create an editable table.
...
```html
<rds-data-table :columns="columns">
  <template slot="data">
    <rds-data-table-row
    ...
    >
      ...
      <rds-data-table-cell>
        <rds-tag :kind="row" label="I am a tag" />
      </rds-data-table-cell>
      <rds-data-table-cell>
        <rds-button type="button" v-html="`Clicky ${row}`" style="width: 100%;"> </rds-button>
      </rds-data-table-cell>
...
- overflow-menu(optional) : An array of overflow menu labels. On click RdsDataTable will raise an 'overflow-menu-click' event passing an object containing menuIndex, menuLabel and rowValue
...
- pagination: (optional) default: false, can be set to true or an object containing camel case props for a RdsPagination component
...
**Overflow menu** To provide customized overflow menus e.g. state dependant, specify explicitly using the slotted data/HTML table form. The RdsDataTable will not raise an event.

## Slots

- batch-actions: Ghost style buttons shown when rows are selected. An array of selected row indexes is returend from the computed property RdsDataTable.selectedRows.
- actions
- data: overrides passing data as a property, expects RdsDataTableRow and RdsDataTableCell. RdsDataTableCell expects slotted content.
- expandedContent: (part of rds-data-table-row) additional content displayed when row is expanded

## Events

- pagination: re-raises RdsPageination change event.
