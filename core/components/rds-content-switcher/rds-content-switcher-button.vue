...
<template>
  <button
    type="button"
    role="tab"
    class="rds-content-switcher-button"
    :class="[
    ...
    @click="open"
  >
    <RdsSvg v-if="icon" :svg="icon" :class="`${carbonPrefix}--content-switcher__icon`" height="16" width="16" />
...
export default {
  ...
  mounted() {
    this.$_RdsContentSwitcherButton = true; // for use by parent with $children

    if (this.contentSelector === '' && this.ownerId === '') {
      console.error('RdsContentSwitcherButton: ownerId or content-selector properties must not be empty strings.');
    }
  ...
  methods: {
    ...
    open() {
      ...
      this.$parent.$emit('rds:open', this);
    },
  },
};
</script>
